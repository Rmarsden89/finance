...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit current liabilities gaps with no supported current canonical "
            "fact using strict raw SEC filing evidence only."
        )
    )
    ...
    parser.add_argument(
        "--raw-sec-subdir",
        default="raw_sec_liabilities_no_supported_current",
    )
...
def main() -> None:
    ...
    cohort_path = base / "liabilities_no_supported_current" / "validation_cohort.csv"
    ...
    if missing:
        raise SystemExit(
            "Missing liabilities raw residual input(s):\n  "
            + "\n  ".join(missing)
        )
    ...
    print("V3 SEC-ONLY NO-SUPPORTED-CURRENT LIABILITIES AUDIT")
