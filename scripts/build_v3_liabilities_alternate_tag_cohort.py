...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the Issue #26 SEC-only cohort for liabilities alternate-tag "
            "research. This does not change canonical liabilities."
        )
    )
...
def main() -> None:
    ...
    cohort["sample_cohort"] = "liabilities_alternate_tag"
    ...
    print("V3 LIABILITIES ALTERNATE-TAG COHORT")
    print(f"As of:                       {args.as_of.isoformat()}")
    print(f"Alternate-tag tickers:       {len(cohort)}")
