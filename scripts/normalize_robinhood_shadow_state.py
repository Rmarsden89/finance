...
def main() -> None:
    ...
    print(f"Orders CSV:              {args.orders_output}")\n    run_log = args.run_log or (args.portfolio_output.parent / "run_log.jsonl")\n    append_run_event(run_log, {"stage":"broker_state_normalize","status":"success","completed_at":utc_now_iso(),"account_value":audit.account_value,"buying_power":audit.buying_power,"positions":audit.positions,"non_final_equity_orders":audit.non_final_equity_orders,"inputs":{"broker_export":artifact_record(args.input)},"outputs":{"portfolio_state":artifact_record(args.portfolio_output),"non_final_orders":artifact_record(args.orders_output)}})\n    print(f"Run log:                 {run_log}")
