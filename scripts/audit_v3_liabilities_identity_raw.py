...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit liabilities identity-gap names for stronger raw SEC evidence "
            "using only direct Liabilities or validated current+noncurrent facts."
        )
    )
    ...
    parser.add_argument(
        "--raw-sec-subdir",
        default="raw_sec_liabilities_identity_raw_strict",
    )
...
def main() -> None:
    ...
    cohort_path = base / "liabilities_identity_raw_strict" / "validation_cohort.csv"
    ...
    if missing:
        raise SystemExit(
            "Missing liabilities identity raw input(s):\n  "
            + "\n  ".join(missing)
        )
    ...
    candidate_mask = detail["status"].isin(
        {
            "raw_direct_liabilities_candidate",
    ...
    summary = {
        ...
        "recoverable_candidate_rows": int(candidate_mask.sum()),
    ...
    detail_path = output_dir / "raw_sec_detail.csv"
    ...
    print("V3 SEC-ONLY IDENTITY-COHORT STRICT RAW LIABILITIES AUDIT")
    print(f"As of:                       {args.as_of.isoformat()}")
    print(f"Cohort rows:                 {summary['cohort_rows']}")
    print(
        f"Recoverable candidates:      "
        f"{summary['recoverable_candidate_rows']}"
    )
    print(
